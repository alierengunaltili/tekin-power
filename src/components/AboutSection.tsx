        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 sm:gap-12 lg:gap-16 items-center">
          ...
          <div ref={contentRef} className="px-4 sm:px-0">
            <div className="space-y-4 sm:space-y-6">
              ...
              {/* Values List */}
              <div ref={valuesRef} className="space-y-2 sm:space-y-3 mt-6 sm:mt-8">
                <h4 className="text-lg sm:text-xl font-bold text-gray-900 mb-3 sm:mb-4">Değerlerimiz:</h4>
                {values.map((value, index) => (
                  <div key={index} className="flex items-start space-x-3 group hover:translate-x-2 transition-transform duration-300">
                    ...
                    <span className="text-sm sm:text-base text-gray-700 group-hover:text-gray-900 transition-colors duration-300">{value}</span>
                  </div>
                ))}
              </div>
              ...
              <div className="pt-4 sm:pt-6">
                <Link href="/hakkimizda" className="bg-gradient-to-r from-blue-600 to-cyan-600 hover:from-blue-700 hover:to-cyan-700 text-white px-6 sm:px-8 py-3 sm:py-4 rounded-xl font-semibold transition-all duration-300 hover:scale-105 hover:shadow-xl inline-flex items-center space-x-2 w-full sm:w-fit justify-center sm:justify-start">
          ...
          {/* Right Stats */}
          <div ref={statsRef} className="grid grid-cols-2 gap-3 sm:gap-4 lg:gap-6 px-4 sm:px-0 mt-8 lg:mt-0">
            {stats.map((stat, index) => {
              const IconComponent = stat.icon;
              return (
                <div
                  key={index}
                  className="group bg-gradient-to-br from-blue-50 to-cyan-50 border border-blue-100 rounded-2xl sm:rounded-3xl p-4 sm:p-6 lg:p-8 text-center hover:shadow-xl hover:scale-105 transition-all duration-500 hover:border-blue-200"
                >
                  <div className="w-12 sm:w-14 lg:w-16 h-12 sm:h-14 lg:h-16 mx-auto mb-3 sm:mb-4 rounded-xl sm:rounded-2xl bg-gradient-to-br from-blue-500 to-cyan-500 p-2.5 sm:p-3 lg:p-4 group-hover:scale-110 group-hover:rotate-6 transition-all duration-500 shadow-lg">
                    <IconComponent className="w-full h-full text-white" />
                  </div>
                  <div className="text-xl sm:text-2xl lg:text-3xl font-bold text-gray-900 mb-1 sm:mb-2 group-hover:text-blue-700 transition-colors duration-300">
                    {stat.number}
                  </div>
